...
void  BSP_SysInit (void)
{
    ...
    SYSTEM.MOFCR.BIT.MODRV2 = 1u;                               /* Set Clk Osc Drive to  20-24 MHz.                     */
    
    SYSTEM.MOSCWTCR.BYTE    = BSP_SYS_MOSCWTCR_WAIT_TIME;       /* Main Clk Osc Wait Time.                              */
    SYSTEM.MOSCCR.BIT.MOSTP = 1u;                               /* Set Main Clk Osc to Operation Mode.                  */
...
//    }
                                                                /* ---------------- PLL CONFIGURATION ----------------- */
    SYSTEM.PLLCR.BIT.PLIDIV = BSP_SYS_PLLCR_PLLDIV_1_2;           /*      - PLIDIV = 1/2                                    */
    SYSTEM.PLLCR.BIT.STC    = BSP_SYS_PLLCR_STC_x12;            /*      - STC    = 12                                   */
                                                                /*      - PLL    = (XTAL_FREQ * PLIDIV) * STC           */
                                                                /*               = (12000000 * 1) * 10                  */
                                                                /*               = 120 MHz                              */
    SYSTEM.PLLCR.BIT.PLLSRCSEL = 1u;                            /* Set main Clk Osc as PPL Clk Source.                  */
...
uint32_t  BSP_SysPerClkFreqGet (void)
{
    ...
    pll_freq = BSP_CFG_SYS_HOCO_CLK_FREQ * pll_stc /2/ pll_div;  //RX65N pll_stc is (.BIT.STC+1/2)
